...
| `cwd` | string | — | — | — | cli/native: the working directory this member's process spawns in, {feature_repo}-substituted. cli: required in practice — the member has no other cwd. native: optional — absent, defaults to the dispatch worktree (or the studio root with no repo-bearing project); a declared value still wins. |
