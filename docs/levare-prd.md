...
**Agents** (`agents/*.md`): `kind: native | cli | remote`. Native: `model`, `skills`, `tools`, `knowledge`, optional `cwd` template (`{feature_repo}`) — absent, defaults to the dispatch worktree (or the studio root with no repo-bearing project). CLI: `command` template with `{task}` substitution, `cwd` template (`{feature_repo}`, required in practice), `timeout`, `result` contract description. Remote: MCP server ref. All: `produces:` — the artifact kinds this member can produce — and `style: { avatar }` (1–2 chars or file). Body = system prompt (native) or wrapper notes. **`produces:` is what makes a member bindable:** the Runner resolves a team's flow step to the one member of that team declaring a matching kind, so an agent that declares nothing can satisfy no step, and a team whose flow step no member can satisfy is a studio that cannot run — rejected by `levare validate`, not discovered at runtime (v1.1; see NOTES F1).
...
**Connectors** (`connectors/*.md`): `kind: mcp | cli`, `server`/`command`, `env:` map of required var *names* (values never present), scope notes, `auth: env | subscription` (NOTES C13, default `env`), `role: model | tool` (NOTES C15, default `tool`). `auth: env` requires a non-empty `env:` — the Runner injects a connector's env only into members whose team/agent grants it, and that grant is the enforcement. `auth: subscription` requires `env:` be empty or absent — the backend authenticates itself from its own stored credentials, and levare does not and cannot scope that credential; `plan:` names the subscription for cost accounting (§10). `role` names what the connector is FOR — `model` for a connector granting model access (e.g. `codex`), `tool` for one granting a service/tool capability (e.g. `github`, `linear`) — orthogonal to `auth`: a `role: model` connector may authenticate via either `env` or `subscription`. A pre-C15 `auth: subscription` connector declaring no explicit `role` is not silently defaulted to `tool` without comment — `levare validate` warns, naming the gap, since that default would mislabel exactly the connector shape C13 exists to describe. **Knowledge** (`knowledge/*.md`): plain documents referenced by name; includes `model-pricing.md` (§10). **Evals** (`evals/*`): golden work units + rubrics (phase-1 fixture doubles as the first).

**Naming convention — `kind` / `type` / `role` (NOTES C15).** Three registry fields sound similar and are not: `kind` answers what a thing is MECHANICALLY — how levare invokes, connects, or parses it (an agent's `native | cli | remote`, a connector's `mcp | cli`, an artifact's frontmatter `kind`). `type` answers which DOMAIN TEMPLATE a thing follows — today, exclusively a work unit's `type: inception | feature | fix | spike | research` (§5). `role` answers what FUNCTION a thing serves in the ecosystem — today, exclusively a connector's `role: model | tool`. The distinction was implicit before this ruling: C13's subscription semantics, the UNKNOWN_MODEL exemption, and doctor's differing failure consequences all reasoned about "model-ish vs. tool-ish" connectors without a field to say so — this ruling names it, and no existing field (agent `kind`, connector `kind`, artifact `kind`, unit `type`) is renamed to make room for it.
...
**Doctor.** `levare doctor` walks connectors and reports required-env presence and CLI auth status before anything runs, plus each connector's `auth` mode (NOTES C13) and `role` (NOTES C15) — and, for any `auth: subscription` connector, a plain warning that levare cannot scope that credential and the grant is documentation, not enforcement. A broken connector's real consequence differs by `role`: a member depending on a missing `role: model` connector cannot start at all; one depending on a missing `role: tool` connector starts and fails mid-work when it reaches for the tool — doctor names which.
