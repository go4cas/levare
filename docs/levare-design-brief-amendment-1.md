...
**Ruling R2 — style direction (Conductor to ratify; a recommendation is offered, not imposed).** The base brief's whole aesthetic is monochrome, minimal, anti-AI-tell, tools-for-craftspeople, with a latent patent-drawing/technical sensibility (the Cogworks lineage). Consistent with that, the **recommended direction is thin geometric line-glyphs** — single-weight strokes, geometric-abstract rather than literal/representational, sitting in the same texture as the mono type and the score rail's own circle/diamond language. Not filled/solid (heavier, more app-icon, less instrument), not skeuomorphic-literal (a tiny robot for "agent" is the AI-tell to avoid). The glyphs should feel *drawn by the same hand that drew the score rail's circles and diamonds* — kin to `◈`/`▸`/`◦`, not imported from an icon pack. **Open decision for the Conductor:** ratify thin-geometric-line, or steer toward filled, or toward the more literal/representational — this single ruling sets the tone for the whole family. The base brief's own hard bans apply regardless: nothing that reads as generated, no default icon-pack tells (no Lucide/Feather-verbatim shapes if they'd be recognised as such), monochrome, 16px-survivable, both modes.
...
## 6. Open decisions for the Conductor to ratify

1. **Icon style (R2)** — thin geometric line-glyphs (recommended, kin to the score rail's circles/diamonds and the mono texture), or filled/solid, or more literal/representational. This single ruling sets the family's whole tone.
2. **Any entity the set is missing** — the ten listed are the base brief's registry vocabulary; confirm none is missing (e.g. does the *Orchestrator* itself, or the *Runner*, want a glyph, or are they deliberately outside the entity set as voice/machinery rather than registry entities?).
3. **Spike / research work-unit glyphs** — still open from the base brief; now also constrained to read as siblings of the entity family, not just of `◈`/`▸`/`◦`.
