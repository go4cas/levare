...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ---------- generic hover/focus tooltip wiring (goal "registry cards legibility" item 2; reused
       for NOTES DOCS-WALKTHROUGH-2's "cited N" tooltip) ----------
       A trigger element carries a tooltip child (matched by `tipSelector` inside it), shown on hover
       AND keyboard focus (`tabindex="0"` on the trigger — never mouseover-only). The trigger commonly
       sits inside an `overflow:hidden` ancestor (`.flowstrip`'s own arrow-clip is load-bearing,
       fault 3) that would clip a plain CSS `position:absolute` tooltip identically — the tooltip is
    ...
    function wireTooltip(triggerSelector, tipSelector) {
      function position(trigger) {
        var tip = trigger.querySelector(tipSelector);
        if (!tip) return;
        var r = trigger.getBoundingClientRect();
        // Below the trigger, not above — assets/styles.css's own note on `.flowstrip .looptip`
        // explains why below is clear of surrounding card content and above routinely wasn't.
